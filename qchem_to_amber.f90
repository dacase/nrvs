program qchem_to_amber

!  converts normal modes from Gaussian, Jaguar or ADF to amber format
!  To run: set on the the input variables below to .true., the recompile
...
!   write out amber-formatted file:

   write(6,'(a)') 'qchem-converted modes'
   write(6,'(i5)') 3*nat
   write(6,'(7f11.5)') x(1:3*nat)
   do i=1,nmodes
      write(6,'(a)') '****'
      write(6,'(i5,f12.5)') i, freq(i)
      write(6,'(7f11.5)') vec(1:3*nat,i)
   end do

end program qchem_to_amber
